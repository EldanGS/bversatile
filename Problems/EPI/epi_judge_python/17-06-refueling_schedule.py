...
def find_ample_city(gallons, distances):
    ...
    city_remaining_gallon_pair = cityAndRemainingGas(0, 0)
    num_cities = len(gallons)
    for i in range(1, num_cities):
        remaining_gallons += gallons[i - 1] - distances[i - 1] // MPG
        if remaining_gallons < city_remaining_gallon_pair.remaining_gallons:
            city_remaining_gallon_pair = cityAndRemainingGas(i, remaining_gallons)
    return city_remaining_gallon_pair.city
