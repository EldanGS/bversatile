...
class LruCache:
    def __init__(self, capacity):
        self._isbn_cache_table = collections.OrderedDict()
    ...
    def insert(self, isbn, price):
        if isbn in self._isbn_cache_table:
            price = self._isbn_cache_table.pop(isbn)
        elif len(self._isbn_cache_table) == self._capacity:
            self._isbn_cache_table.popitem(last=False)
        self._isbn_cache_table[isbn] = price

    def erase(self, isbn):
        return self._isbn_cache_table.pop(isbn, None) is not None
